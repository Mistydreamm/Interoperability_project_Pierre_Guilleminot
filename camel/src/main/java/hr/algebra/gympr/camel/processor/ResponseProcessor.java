package hr.algebra.gympr.camel.processor;
...
public class ResponseProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        envelope.put("capturedAt", LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
        ...
        try {
            envelope.set("response", mapper.readTree(rawBody.isEmpty() ? "null" : rawBody));
        } catch (Exception e) {
            envelope.put("response", rawBody);
        }

        String envelopeJson = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(envelope);
